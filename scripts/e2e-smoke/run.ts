...
import type { RenderBatchOptions, RenderClipJob } from '../../src/main/render/types';
...
function pickSegments(words: WordTimestamp[], videoDuration: number): PickedSegment[] {
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    const idx = Math.max(
      0,
      words.findIndex((w) => w.start >= target),
    );
...
async function renderOne(
  ...
  log(
    'Render',
    `→ ${entry.archetype} / ${entry.captionMode} → ${basename(outPath)} (${(endTime - startTime).toFixed(1)}s)`,
  );
...
function readPPM(path: string): { width: number; height: number; pixels: Uint8Array } {
  ...
  function readToken(): string {
    while (
      off < buf.length &&
      (buf[off] === 0x20 || buf[off] === 0x0a || buf[off] === 0x0d || buf[off] === 0x09)
    )
      off++;
    // skip comments
...
function verifyClip(entry: RenderPlanEntry, outputPath: string): ClipVerification {
  ...
  if (!isQuote && !creamPresent) {
    failures.push(
      `Cream caption color #f6ecd9 not found in caption band across sampled frames (closest=${bestCream.closestHex}, dist=${bestCream.closestDist.toFixed(0)})`,
    );
  }
  ...
  if (!isQuote && entry.captionMode === 'emphasis_highlight' && !accentPresent) {
    failures.push(
      `Accent #9f75ff not found in caption band for emphasis_highlight clip (closest=${bestAccent.closestHex})`,
    );
  }
...
async function tryTranscribe(
  videoPath: string,
): Promise<{ result: TranscriptionResult; source: 'parakeet' | 'srt-fallback' }> {
  try {
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < picks.length; i++) {
    ...
    log(
      'Score',
      `  Clip ${i + 1}: t=${p.startTime.toFixed(1)}–${p.endTime.toFixed(1)}s · score=${p.score} · hook="${p.hookText}"`,
    );
  }
      ...
      segmentedSegments: [
        {
          ...
          transitionIn:
            ARCHETYPE_DEFAULT_TRANSITION_IN[entry.archetype] ?? editStyle.defaultTransition,
          accentColor: BRAND_ACCENT,
        },
      ],
  ...
  const clipResults = new Map<string, string | null>(
    renderResults.map((r) => [r.entry.clipId, r.outputPath]),
  );
  const clipRenderTimes = new Map<string, number>(
    renderResults.map((r) => [r.entry.clipId, r.renderTimeMs]),
  );
  ...
  for (const r of renderResults) {
    ...
    log(
      'Verify',
      `  ${v.ok ? 'OK  ' : 'FAIL'} ${v.clipId} ${v.archetype}/${v.captionMode} ${v.width}×${v.height}@${v.fps} cream=${v.creamPresent} accent=${v.accentPresent}` +
        (v.sandBgPresent !== undefined ? ` bg=${v.sandBgPresent}` : ''),
    );
  ...
  try {
    const parsed = JSON.parse(readFileSync(jsonPath, 'utf-8')) as {
      clips: { id: string; accentColor?: string }[];
    };
    if (parsed.clips.length !== 3)
      manifestErrors.push(`manifest.json has ${parsed.clips.length} clips, expected 3`);
...
function writeReport(r: ReportInput): void {
  ...
  lines.push('');
  lines.push(
    `**Result:** ${r.allOk ? '✅ all checks passed' : '❌ failures present (see Issues)'}`,
  );
  lines.push('');
  ...
  lines.push(
    `| Score       | ✅ stub | Gemini key not present in this env — used local deterministic picker matching the \`ScoredSegment\` shape. Picked ${r.picks.length} clips. |`,
  );
  lines.push(`| Approve     | ✅ | All 3 picks approved |`);
  lines.push(
    `| Render      | ${r.renderResults.every((x) => x.outputPath) ? '✅' : '❌'} | renderSegmentedClip × ${r.renderResults.length} (PRESTYJ) |`,
  );
  lines.push(
    `| Manifest    | ${r.manifestErrors.length === 0 ? '✅' : '❌'} | manifest.json + manifest.csv at \`${r.manifestPath}\` |`,
  );
  lines.push('');
  ...
  for (let i = 0; i < r.plan.length; i++) {
    ...
    const pk = p.pick;
    lines.push(
      `| ${i + 1} | ${pk.startTime.toFixed(1)}–${pk.endTime.toFixed(1)} | \`${p.archetype}\` | \`${p.captionMode}\` | ${pk.hookText} |`,
    );
  }
  lines.push('');
  ...
  for (const v of r.verifications) {
    ...
    const accent =
      v.captionMode === 'emphasis_highlight'
        ? v.accentPresent
          ? `✅ (closest ${v.accentClosest})`
          : `❌ ${v.accentClosest}`
        : v.accentPresent
          ? `+ (closest ${v.accentClosest})`
          : `n/a (closest ${v.accentClosest})`;
    const bg =
      v.sandBgPresent === undefined
        ? 'n/a'
        : v.sandBgPresent
          ? `✅ (closest ${v.sandBgClosest})`
          : `❌ ${v.sandBgClosest}`;
    lines.push(
      `| ${v.clipId} | \`${out}\` | ${v.width}×${v.height} | ${v.fps} | ${cream} | ${accent} | ${bg} | ${v.ok ? '✅' : '❌'} |`,
    );
  }
  lines.push('');
  ...
  lines.push('');
  lines.push(
    `- Backdrop: \`${BRAND_FG}\` sand bg (inverted brand palette for fullscreen-quote). Verified by sampling the upper region of the frame.`,
  );
  lines.push(
    `- Caption text: \`${BRAND_BG}\` dark-brown Instrument Serif Italic on the sand bg. Caption-band cream test is skipped for this archetype — the cream is the bg, not the text.`,
  );
  lines.push(
    `- Note: emphasis recolor is suppressed for fullscreen-quote so the inverted palette stays clean.`,
  );
  lines.push('');
  lines.push('## Manifest');
  lines.push('');
