...
const JOBS: ArchetypeJob[] = [
  // 1. Intro (fullscreen-headline) — big title + dark brand bg.
  {
    ...
    outputName: 'render_fullscreen-headline_intro.mp4',
    overlayText: '1 thing\nmake Claude\nbetter',
  },
  ...
  // 4. Just subtitles example → fullscreen-quote (solid bg + center text).
  {
    ...
    outputName: 'render_fullscreen-quote.mp4',
    overlayText: 'Claude keeps\nmissing the mark',
  },
...
async function renderOne(
  job: ArchetypeJob,
  allWords: { text: string; start: number; end: number }[],
): Promise<string> {
...
async function main(): Promise<void> {
  ...
  for (const r of results) {
    console.log(
      `${r.ok ? 'OK  ' : 'FAIL'} ${r.job.archetype.padEnd(22)} ${r.ok ? r.path : r.error}`,
    );
  }
