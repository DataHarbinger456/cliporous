...
const STYLE_IMAGE_GUIDANCE: Record<string, string> = {
  viral:
    'Vibrant, high-contrast, bold colors, dynamic composition, eye-catching, social media aesthetic',
  educational:
    'Clean, informative, flat illustration style, clear visual hierarchy, infographic-like',
  cinematic:
    'Warm and cinematic, film grain, shallow depth of field, dramatic lighting, golden hour tones',
...
function _classifyGeminiError(err: unknown): never {
...
async function callGeminiImageApi(prompt: string, geminiApiKey: string): Promise<Buffer> {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      `Gemini Image API error: ${response.status} ${response.statusText} — ${errBody}`,
    );
...
export async function generateBRollImages(
  ...
  for (let i = 0; i < requests.length; i += CONCURRENCY) {
    ...
    const settled = await Promise.allSettled(
      batch.map((req) =>
        generateBRollImage(req.keyword, req.transcriptContext, styleCategory, geminiApiKey),
      ),
    );
