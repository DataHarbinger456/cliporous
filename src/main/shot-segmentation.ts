...
export type { ShotBreakReason, ShotSegment, ShotSegmentationResult };
...
function detectBreaks(words: WordTimestamp[], config: SegmentationConfig): BreakCandidate[] {
  ...
  for (let i = 0; i < words.length - 1; i++) {
    ...
    if (endsSentence(current.text)) {
      const confidence =
        gap > config.strongPauseThreshold ? 0.95 : gap > config.pauseThreshold ? 0.85 : 0.7;
      breaks.push({
        wordIndex: i,
  ...
  if (words.length >= config.topicWindowSize * 2) {
    ...
    for (let i = half; i < words.length - half; i++) {
      ...
      const afterVocab = getVocabulary(
        words,
        i,
        Math.min(words.length, i + config.topicWindowSize),
      );
      ...
      if (similarity < config.topicShiftThreshold) {
        ...
        const breakTime = i < words.length - 1 ? words[i].end + gap / 2 : words[i].end;
...
function forceSplitOversized(
  ...
  for (const shot of shots) {
    ...
    for (let i = shot.startWordIndex + 1; i < shot.endWordIndex; i++) {
      ...
      const bonus = endsSentence(words[i - 1].text)
        ? -0.5
        : endsClause(words[i - 1].text)
          ? -0.3
          : 0;
    ...
    const splitTime = bestIdx > 0 && bestIdx < words.length ? words[bestIdx].start : midTime;
    ...
    result.push({
      ...
      endTime: splitTime,
      text: words
        .slice(shot.startWordIndex, bestIdx)
        .map((w) => w.text)
        .join(' '),
    ...
    const secondHalf: ShotSegment = {
      ...
      text: words
        .slice(bestIdx, shot.endWordIndex)
        .map((w) => w.text)
        .join(' '),
 ...
 */
function mergeUndersized(shots: ShotSegment[], config: SegmentationConfig): ShotSegment[] {
