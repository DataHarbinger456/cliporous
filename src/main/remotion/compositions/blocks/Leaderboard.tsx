/**
 * Leaderboard — ranked Top-N rows with a rank Chip, a label, and a value.
 *
 * A *content block*: it composes a `BlockSkin` (via `skinId`) for its look so
 * the same block renders in every skin. Inner rows are built from plain divs +
 * a shadcn `Separator`; all motion is frame-clock driven via
 * spring()/interpolate().
 */
...
export const Leaderboard: React.FC<LeaderboardProps> = ({
  ...
  // Row = 62px chip + 32px gap + label + 32px gap + value column.
  const rowText = cw - 62 - VALUE_WIDTH - 32 * 2 - 8;
