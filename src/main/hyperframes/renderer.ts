...
import { type RenderCompositionResult, renderComposition } from './engine';
import type {
  HyperFramePreset,
  OverlayBlockName,
  OverlayRenderResult,
...
function buildVariables(request: OverlayRequest): Record<string, unknown> {
  ...
  switch (block) {
    ...
    case 'promo-agent-toast':
      vars.message =
        (props as Record<string, unknown>).message ?? props.text ?? 'Generated 15 on-brand images';
    ...
    case 'promo-chat-exchange':
      ...
      vars.userText =
        (props as Record<string, unknown>).userText ??
        props.text ??
        'Write 5 captions in our tone of voice';
      vars.agentText =
        (props as Record<string, unknown>).agentText ?? 'Wrote 5 captions in your tone';
      vars.appName = (props as Record<string, unknown>).appName ?? 'Media Master';
    ...
    case 'promo-publish':
      vars.label =
        (props as Record<string, unknown>).label ?? props.text ?? 'Publish to 9 platforms';
    ...
    case 'promo-feature-flash':
      ...
      vars.description =
        ...
        'Turn winning competitor ads into on-brand posts';
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;
    ...
    case 'delos-scan-result':
      ...
      vars.findings = (props as Record<string, unknown>).findings ?? [
        'Fidelity normal',
        'No anomalies',
        'Memory intact',
      ];
    ...
    // Media Master Promo Mode evidence templates
    case 'promo-repurpose-stack':
      vars.sourceLabel = (props as Record<string, unknown>).sourceLabel ?? 'ONE IDEA';
      vars.items = (props as Record<string, unknown>).items ?? [];
      vars.outputLabel = (props as Record<string, unknown>).outputLabel ?? 'EVERYWHERE';
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-brand-score':
      vars.score = (props as Record<string, unknown>).score ?? 94;
      vars.label = (props as Record<string, unknown>).label ?? 'BRAND CONSISTENCY';
      vars.checks = (props as Record<string, unknown>).checks ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-content-calendar':
      vars.month = (props as Record<string, unknown>).month ?? 'THIS WEEK';
      vars.days = (props as Record<string, unknown>).days ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-platform-export':
      vars.label = (props as Record<string, unknown>).label ?? 'READY TO PUBLISH';
      vars.platforms = (props as Record<string, unknown>).platforms ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-hook-test':
      vars.title = (props as Record<string, unknown>).title ?? 'HOOK TEST';
      vars.variants = (props as Record<string, unknown>).variants ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-before-after':
      vars.beforeLabel = (props as Record<string, unknown>).beforeLabel ?? 'BEFORE';
      vars.beforeText = (props as Record<string, unknown>).beforeText ?? '';
      vars.afterLabel = (props as Record<string, unknown>).afterLabel ?? 'AFTER';
      vars.afterText = (props as Record<string, unknown>).afterText ?? '';
      vars.beforeMetric = (props as Record<string, unknown>).beforeMetric ?? '';
      vars.afterMetric = (props as Record<string, unknown>).afterMetric ?? '';
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-analytics-spike':
      vars.metric = (props as Record<string, unknown>).metric ?? 'REACH';
      vars.value = (props as Record<string, unknown>).value ?? '248K';
      vars.delta = (props as Record<string, unknown>).delta ?? '+42%';
      vars.points = (props as Record<string, unknown>).points ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-caption-sync':
      vars.label = (props as Record<string, unknown>).label ?? 'CAPTION SYNC';
      vars.words = (props as Record<string, unknown>).words ?? [];
      vars.activeIndex = (props as Record<string, unknown>).activeIndex ?? 0;
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-testimonial-card':
      vars.quote = (props as Record<string, unknown>).quote ?? '';
      vars.name = (props as Record<string, unknown>).name ?? 'Creator';
      vars.role = (props as Record<string, unknown>).role ?? '';
      vars.result = (props as Record<string, unknown>).result ?? '';
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-approval-flow':
      vars.title = (props as Record<string, unknown>).title ?? 'APPROVAL FLOW';
      vars.steps = (props as Record<string, unknown>).steps ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-brand-kit-scan':
      vars.title = (props as Record<string, unknown>).title ?? 'BRAND KIT SCAN';
      vars.checks = (props as Record<string, unknown>).checks ?? [];
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;

    case 'promo-content-lottery':
      vars.title = (props as Record<string, unknown>).title ?? 'PICK YOUR NEXT POST';
      vars.items = (props as Record<string, unknown>).items ?? [];
      vars.winnerIndex = (props as Record<string, unknown>).winnerIndex ?? 0;
      vars.accentColor = (props as Record<string, unknown>).accentColor ?? '#1d3557';
      break;
  }
 ...
 */
export async function renderOverlays(requests: OverlayRequest[]): Promise<OverlayRenderResult[]> {
  ...
  for (const request of requests) {
    try {
      ...
      console.warn(`[HyperFrames] Failed to render overlay block "${request.block}": ${message}`);
      results.push({
        movPath: '',
        duration: request.timing.duration,
        width: OUTPUT_WIDTH,
