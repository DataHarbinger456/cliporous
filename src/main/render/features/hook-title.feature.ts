...
import { buildASSFilter, cssHexToASS, formatASSTimestamp } from '../helpers';
import type { HookTitleConfig, RenderBatchOptions, RenderClipJob } from '../types';
...
export function generateHookTitleASSFile(
  ...
  const assPath = join(
    tmpdir(),
    `${filenamePrefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}.ass`,
  );
...
export function createHookTitleFeature(): RenderFeature {
  ...
  return {
    ...
    async prepare(
      ...
      const hookEnabled = ov === undefined ? (batchOptions.hookTitleOverlay?.enabled ?? false) : ov;
      ...
      if (!job.hookTitleConfig?.enabled || !job.hookTitleText) {
        if (hookEnabled && !job.hookTitleText) {
          console.warn(
            `[HookTitle] Clip ${job.clipId} has no hookTitleText — hook overlay will be skipped`,
          );
        }
      ...
      try {
        ...
        const yPositionPx =
          isSpeakerFullscreen(hookArchetype) && batchOptions.templateLayout?.titleText
