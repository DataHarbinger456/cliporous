...
export const hyperframesOverlayFeature: RenderFeature = {
  ...
  async prepare(
    ...
    // No overlays requested or creator disabled them for this clip.
    if (
      batchOptions.hyperframesEnabled === false ||
      job.clipOverrides?.enableHyperframes === false
    ) {
      job.hyperframesOverlays = undefined;
  ...
  async postProcess(
    ...
    if (
      ...
      job.hyperframesOverlays.length === 0
    ) {
      return renderedPath;
    }
...
export async function populateDelosCardContent(
  ...
  try {
    const content = await buildCardContent(kind, windowText, windowWords, apiKey ? { apiKey } : {});
    const slots = cardContentToSlots(content);
    if (!hasMeaningfulContent(slots)) {
      console.warn(`[HyperFrames] Card content for ${kind} was empty, keeping preset defaults`);
      return;
    }
