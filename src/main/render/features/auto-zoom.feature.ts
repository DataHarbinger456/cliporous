...
import type { ZoomMode, ZoomIntensity, ShotStyleConfig } from '@shared/types'
...
export function computeIdleIntervals(
  busy: TimeInterval[],
  clipDuration: number
): TimeInterval[] {
...
function keyframeCoverage(
  keyframes: EmphasisKeyframe[],
...
export function idleZoomShotConfigs(shots: IdleZoomShot[]): ShotStyleConfig[] {
  return shots.map((s, i) => ({
    ...
    zoom: {
      ...
      intervalSeconds:
        s.direction === 'in'
          ? IDLE_ZOOM_INTERVAL_IN_SECONDS
          : IDLE_ZOOM_INTERVAL_HOLD_SECONDS
    }
  }))
}
