...
export const accentColorFeature: RenderFeature = {
  ...
  async postProcess(
    ...
    const snapshot = (job as unknown as Record<string, unknown>).__accentSnapshot as
      | BatchSnapshot
      | undefined;
 ...
 */
export function restoreBatchOptions(job: RenderClipJob, batchOptions: RenderBatchOptions): void {
  const snapshot = (job as unknown as Record<string, unknown>).__accentSnapshot as
    | BatchSnapshot
    | undefined;
  if (!snapshot) return;
